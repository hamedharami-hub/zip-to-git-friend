      ...
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
        <DropdownMenuTrigger asChild>
          <Button
            size="icon"
            variant="ghost"
            className="h-7 w-7 opacity-60 sm:opacity-0 sm:group-hover:opacity-100"
          >
    ...
    <div className="space-y-2">
      ...
      {ungrouped.length > 0 && (
        <ul className="space-y-0.5">
          {folders.length > 0 && (
            <li className="px-1 py-0.5 text-[10px] uppercase text-muted-foreground tracking-wider">
              بدون پوشه
            </li>
          )}
          {ungrouped.map((s) => (
