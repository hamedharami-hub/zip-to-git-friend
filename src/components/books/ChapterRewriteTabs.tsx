...
import { REWRITE_STYLES, rewriteErrorMessage } from "@/lib/chapterRewrite";
import { rewriteChapterRouted } from "@/lib/bookAiRouter";
import { coerceBookModel } from "@/lib/aiModels";
import { deleteChapterRewrite, getRewritesForChapter } from "@/lib/bookDb";
import { InteractiveBookText } from "@/components/books/InteractiveBookText";
import type { BookChapterRewrite, RewriteStyle } from "@/types";
...
export function ChapterRewriteTabs({
  ...
  const [activeStyle, setActiveStyle] = useState<RewriteStyle>(REWRITE_STYLES[0].id);
  ...
  const hasAnyRewrite = useMemo(() => Object.values(rewrites).some((r) => !!r), [rewrites]);
  ...
  return (
    <section
      id="chapter-rewrite"
      className="mt-12 pt-8 border-t border-border/50 not-prose scroll-mt-24"
    >
      ...
      <p className="text-[11px] text-muted-foreground mt-3" dir="auto">
        نکته: همه بازنویسی‌ها در همین مرورگر ذخیره می‌شوند. هر سبک یک‌بار ساخته می‌شود و دفعات بعد
        بلافاصله از حافظه باز می‌شود.
      </p>
    </section>
  );
}
