import {
  Newspaper, Loader2, Clock, RefreshCw, Folder, Globe2,
} from 'lucide-react';
...
function FeedItemCard({ item, onOpen, titleFa }: { item: EnrichedItem; onOpen: (item: FeedItem) => void; titleFa?: string }) {
  const seen = isSeen(item.url);
  return (
    <li id={`news-item-${encodeURIComponent(item.url)}`} className="scroll-mt-24 rounded-xl transition-shadow">
      <button
        type="button"
        onClick={() => onOpen(item)}
        className={
...
export function FolderAggregatedView({
  folder, items, loading, onRefresh, onOpenItem, sources,
}: {
    ...
    <>
      ...
      {items.length === 0 ? (
        ...
        <ul className="space-y-3">
          {items.map((item) => (
            <FeedItemCard key={item.url} item={item} onOpen={onOpenItem} titleFa={titleTr[item.url]?.titleFa} />
          ))}
        </ul>
      )}
    </>
