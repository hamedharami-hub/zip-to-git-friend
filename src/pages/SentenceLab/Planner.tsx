...
import { ArrowLeft, Sparkles, Loader2, Play, Trash2, Wand2, Check, ChevronRight } from 'lucide-react';
...
import {
  saveScenario, listScenarios, deleteScenario, type CachedScenario, type ScenarioStep,
...
export default function SentencePlanner() {
  ...
  async function generate() {
    ...
    try {
      ...
      const modelId = sentenceLabModelRef?.provider === 'gateway' ? sentenceLabModelRef.model : undefined;
      ...
      <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur">
        <div className="container mx-auto flex items-center gap-2 px-4 py-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">سناریوی ترکیبی از فولدرهای انتخاب‌شده</p>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto space-y-6 px-4 py-6">
        <Card>
          <CardContent className="space-y-4 pt-6">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{c.sentenceCount} sentences</div>
                      </div>
                    </div>
            ...
            <Button onClick={generate} disabled={generating || selected.size === 0} className="w-full">
        ...
        <div>
          ...
          {saved.length === 0 ? (
            ...
            <div className="space-y-2">
              {saved.map((s) => (
                <Card key={s.id} className="cursor-pointer hover:bg-muted/40" onClick={() => setActive(s)}>
                  <CardContent className="flex items-center justify-between gap-2 py-3">
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-medium">{s.title}</div>
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Badge variant="secondary" className="text-[10px]">{s.steps.length} steps</Badge>
                        {s.topic && <span className="truncate">{s.topic}</span>}
                      </div>
                    </div>
                    <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setActive(s); }}>
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur">
        <div className="container mx-auto flex items-center gap-2 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-muted-foreground">Step {idx + 1} / {total} · {progress}%</div>
          </div>
        </div>
      ...
      <main className="container mx-auto max-w-2xl space-y-4 px-4 py-6">
        {scenario.scenario && idx === 0 && (
          <Card><CardContent className="py-4 text-sm text-muted-foreground">{scenario.scenario}</CardContent></Card>
        )}

        <Card>
          <CardContent className="space-y-4 pt-6">
            <div>
              <div className="mb-1 text-xs font-semibold uppercase text-muted-foreground">موقعیت</div>
              <div dir="rtl" className="text-base leading-relaxed">{step.prompt_fa}</div>
              {step.prompt_en && (
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                <div className="text-xs font-semibold uppercase text-muted-foreground">Your line</div>
        ...
        <div className="flex gap-2">
          <Button variant="outline" disabled={idx === 0} onClick={() => { setIdx(idx - 1); setRevealed(false); }} className="flex-1">
            Prev
          </Button>
          {idx < total - 1 ? (
            <Button onClick={() => { setIdx(idx + 1); setRevealed(false); }} className="flex-1">
              Next <ChevronRight className="h-4 w-4" />
            </Button>
          ) : (
            <Button onClick={onClose} className="flex-1">Finish</Button>
          )}
        </div>
      </main>
    </div>
