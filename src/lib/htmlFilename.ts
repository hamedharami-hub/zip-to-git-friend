...
async function suggestWithGemini(apiKey: string, model: string, userPrompt: string): Promise<string> {
  ...
  return cleanFilename(data?.candidates?.[0]?.content?.parts?.map((p: any) => p?.text ?? '').join(' ') ?? '');
}
...
export async function suggestPersianHtmlFilename(args: {
  ...
  const ref = coerceBookModel(args.modelRef ?? settings.htmlFilenameModelRef ?? 'google/gemini-3.1-flash-lite-preview');
  const userPrompt = prompt(args.title, args.excerpt, args.siteName);

  if (ref.provider === 'gateway') {
    const { data, error } = await supabase.functions.invoke<{ filename?: string; error?: string }>('suggest-html-filename', {
      body: { title: args.title, excerpt: args.excerpt, siteName: args.siteName, url: args.url, model: ref.model },
    });
