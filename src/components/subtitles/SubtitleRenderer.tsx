...
export function SubtitleRenderer({
  ...
  const showInlineTranslation = useSettingsStore(
    (s) => s.settings.showInlineTranslation,
  );
    ...
    return (
      cards.find(
        (c) =>
          ...
          (c.sourceCueId === primaryCue.id ||
            c.sourceCueId?.startsWith(`${primaryCue.id}@`)),
      ) ?? null
    );
