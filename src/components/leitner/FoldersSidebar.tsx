...
export function FoldersSidebar({ selectedId, onSelect, onReview }: Props) {
  ...
  const [editing, setEditing] = useState<
    null | { mode: "create" } | { mode: "rename"; folder: LeitnerFolder }
  >(null);
  ...
  const dueCount = (folderId?: string) => {
    const now = Date.now();
    return cards.filter((c) => (folderId ? c.folderId === folderId : true) && c.nextReview <= now)
      .length;
  };
  ...
  const openRename = (f: LeitnerFolder) => {
    setName(f.name);
    setEditing({ mode: "rename", folder: f });
  };
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {editing?.mode === "create"
                ? "یک نام برای پوشه‌ی جدید انتخاب کن."
                : "نام جدید پوشه را وارد کن."}
            </DialogDescription>
          </DialogHeader>
          <Input
            autoFocus
