        ...
        <button className="flex items-center gap-2 rounded-full border bg-card px-2 py-1 text-xs hover:bg-muted/60 transition-colors">
          ...
          {!compact && (
            <span className="hidden sm:flex items-center gap-1 text-muted-foreground">
              <Flame className={`h-3.5 w-3.5 ${state.currentStreak > 0 ? 'text-orange-500' : ''}`} />
              <span className="tabular-nums">{state.currentStreak}</span>
            </span>
          )}
          <span className="flex items-center gap-1">
            <Heart className={`h-3.5 w-3.5 ${state.hearts > 0 ? 'fill-rose-500 text-rose-500' : 'text-muted-foreground'}`} />
            <span className="tabular-nums">{state.hearts}</span>
          </span>
          {combo >= 3 && (
            <span className={`flex items-center gap-0.5 rounded-full px-1.5 font-bold ${
              combo >= 10 ? 'bg-amber-500/20 text-amber-400' : combo >= 5 ? 'bg-purple-500/20 text-purple-400' : 'bg-sky-500/20 text-sky-400'
            }`}>
      ...
      <PopoverContent align="end" className="w-80 p-3">
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-muted-foreground tabular-nums">{p.into}/{p.span} XP</span>
          </div>
        ...
        {/* Quests */}
        <p className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1.5">Daily Quests</p>
            ...
            quests.map((q) => {
              ...
              return (
                <div key={q.id} className="rounded-md border p-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-medium" dir="rtl">{q.title}</span>
                  ...
                  <div className="mt-1.5 flex items-center justify-between">
                    ...
                    {q.completed && !q.claimed ? (
                      <Button size="sm" className="h-6 px-2 text-[10px]" onClick={() => claim(q.id)}>
                        دریافت 🎁
                      </Button>
                    ) : q.claimed ? (
