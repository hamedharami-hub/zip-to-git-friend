...
function sanitizeChapterHtml(raw: string): { html: string; text: string } {
  ...
  doc.querySelectorAll('script, style, link, meta, iframe, object, embed').forEach((n) =>
    n.remove(),
  );
...
export async function parseEpub(
  ...
  const getMeta = (tag: string): string | undefined => {
    ...
    const el = opfDoc.getElementsByTagName(`dc:${tag}`)[0]
      ?? opfDoc.getElementsByTagNameNS('http://purl.org/dc/elements/1.1/', tag)[0]
      ?? opfDoc.getElementsByTagName(tag)[0];
    return el?.textContent?.trim() || undefined;
  };
  const title = dedupeTitle(getMeta('title'), (file as File).name?.replace(/\.epub$/i, '') ?? 'Untitled');
