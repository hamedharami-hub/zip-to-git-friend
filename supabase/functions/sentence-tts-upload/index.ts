...
Deno.serve(async (req) => {
  ...
  if (head.ok) {
    return new Response(JSON.stringify({ url: publicUrl(SUPABASE_URL, path), cached: true }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  return new Response(JSON.stringify({ url: publicUrl(SUPABASE_URL, path), cached: false }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
});
