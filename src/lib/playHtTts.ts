...
export async function synthesizeWithPlayHt(params: {
  ...
  if (!userId?.trim() || !apiKey?.trim())
    throw new PlayHtTtsError("auth", "Play.ht user/key کامل نیست.");
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403)
      throw new PlayHtTtsError("auth", `Play.ht 401: ${txt}`);
