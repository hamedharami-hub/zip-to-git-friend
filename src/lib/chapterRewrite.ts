...
import { getChapterRewrite, rewriteKey, saveChapterRewrite } from "@/lib/bookDb";
...
export async function rewriteChapter(
  ...
  const { data, error } = await supabase.functions.invoke<RewriteResponse>("rewrite-chapter", {
    body: {
      chapterText,
      chapterTitle,
      style,
