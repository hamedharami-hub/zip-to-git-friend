...
export class HuggingFaceTtsError extends Error {
  constructor(public code: 'auth' | 'cold' | 'quota' | 'other', msg: string) { super(msg); }
}
...
export async function synthesizeWithHuggingFace(params: {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) throw new HuggingFaceTtsError('auth', `HF 401: ${txt}`);
    if (res.status === 503) throw new HuggingFaceTtsError('cold', 'مدل HF در حال warm-up است؛ چند ثانیه بعد دوباره امتحان کن.');
