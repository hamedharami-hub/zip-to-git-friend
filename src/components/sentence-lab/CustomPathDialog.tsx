...
export function CustomPathDialog({ open, onOpenChange, onCreated, domain = 'general' }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (catSlugs.length === 0) { setOptions([]); return; }
  ...
  async function save() {
    if (!name.trim()) { toast({ title: 'لطفاً یک نام بگذار', variant: 'destructive' }); return; }
    if (steps.length === 0) { toast({ title: 'حداقل یک منبع اضافه کن', variant: 'destructive' }); return; }
    setSaving(true);
    try {
      await createPath({
        name: name.trim(),
      ...
      setName(''); setDescription(''); setSteps([]); setPickValue('');
      onCreated();
    } catch (e: any) {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3">
          <div>
            ...
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="مثلاً: قبل از سفر" />
          </div>
          <div>
          ...
          <div className="rounded-lg border p-2.5">
            ...
            <div className="mt-2 flex gap-2">
              <Select value={pickValue} onValueChange={setPickValue}>
                <SelectTrigger className="flex-1"><SelectValue placeholder="انتخاب زیرشاخه..." /></SelectTrigger>
              ...
              <Input
                type="number" min={1} max={50} value={pickCount}
              ...
              <div className="mt-3 space-y-1.5">
                {steps.map((s, i) => (
                  <div key={i} className="flex items-center justify-between rounded-md bg-muted/50 px-2.5 py-1.5">
                    <span className="text-xs">{s.category} / {s.subcategory}</span>
