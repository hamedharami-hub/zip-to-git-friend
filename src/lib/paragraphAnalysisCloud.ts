...
export async function saveParagraphAnalysisShared(record: BookParagraphAnalysis): Promise<void> {
    ...
    await supabase.from("paragraph_analyses").upsert(
      {
        ...
        vocabulary: (record.vocabulary ?? []) as unknown as never,
        idioms: (record.idioms ?? []) as unknown as never,
