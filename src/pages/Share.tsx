...
const SharePage = () => {
  ...
  useEffect(() => {
    ...
    const directUrl =
      sharedUrlParam ||
      sharedTextParam.match(/https?:\/\/\S+/i)?.[0] ||
      '';
    if (!ids && directUrl) {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        const filename = decodeURIComponent(
          res.headers.get('X-Shared-Filename') || `shared-${id}`,
        );
          ...
          {(status === 'idle' || status === 'fallback') && (
            <>
              ...
              <p className="text-sm text-muted-foreground">
                Pick a video, audio file, or <code>.llp</code> learning pack.
                On Android you can also share directly into this app from another app.
              </p>
