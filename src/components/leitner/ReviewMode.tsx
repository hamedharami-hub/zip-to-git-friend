...
const RATINGS: Array<{ key: LeitnerRating; label: string; hotkey: string; cls: string }> = [
  ...
  },
  {
    ...
    label: "Good",
    hotkey: "3",
    cls: "border-emerald-500/40 text-emerald-600 hover:bg-emerald-500/10",
  },
  {
    key: "easy",
    label: "Easy",
    hotkey: "4",
    cls: "border-sky-500/40 text-sky-600 hover:bg-sky-500/10",
  },
];
    ...
    >
      ...
      {!compact && (
        <div className="space-y-1">
          ...
          <div className="flex items-center justify-between text-[10px] text-muted-foreground tabular-nums">
            <span>
              {initialSize - queue.length} / {initialSize}
            </span>
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span className="inline-flex items-center gap-1.5">
          ...
          {current.cefr && (
            <span className="ml-1 px-1.5 py-0.5 rounded bg-muted">{current.cefr}</span>
          )}
      ...
      {!compact && current.imageUrl && !youtubeIdFromUrl(current.sourceUrl) && (
        <img
          ...
          alt=""
          className="rounded-lg w-full max-w-xs mx-auto aspect-square object-cover"
          loading="lazy"
        />
      )}
      ...
      >
        ...
        {!compact && revealed && (swipeDx !== 0 || swipeDy !== 0) && (
          <div className="absolute inset-0 pointer-events-none flex items-center justify-between px-2 text-[11px] font-semibold">
            <span
              className={`px-2 py-1 rounded-full bg-destructive/15 text-destructive transition-opacity ${swipeDx < -30 ? "opacity-100" : "opacity-30"}`}
            >
              ← Again
            </span>
            <span
              className={`px-2 py-1 rounded-full bg-emerald-500/15 text-emerald-600 transition-opacity ${swipeDx > 30 ? "opacity-100" : "opacity-30"}`}
            >
              Good →
            </span>
          </div>
        ...
        {!compact && (
          <div className="flex flex-wrap items-center justify-center gap-1">
            ...
            >
              ...
              {current.audioUrl ? "Play clip" : "Pronounce"}
            </Button>
            {current.audioUrl && (
              <Button
      ...
      {(effectiveMode === "type" || effectiveMode === "cloze") &&
        revealed &&
        typedResult !== "pending" && (
          <div
          ...
          >
            {typedResult === "correct"
              ? "✓ آفرین!"
              : `✗ جواب: ${effectiveMode === "cloze" ? cloze?.answer : current.back}`}
          </div>
        )}
