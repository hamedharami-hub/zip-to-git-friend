...
export function useOtherEngineVoices(ttsLang: "en" | "fa"): OtherEngineVoices {
  ...
  const [playHtVoice, setPlayHtVoice] = useState<string>(() => {
    try {
      return localStorage.getItem(PLAYHT_KEY) || PLAYHT_VOICES[0].id;
    } catch {
      return PLAYHT_VOICES[0].id;
    }
  });
  const [openTtsVoice, setOpenTtsVoice] = useState<string>(() => {
    try {
      ...
      return "larynx:en-us/ek-glow_tts";
    }
  });
  ...
  useEffect(() => {
    try {
      if (azureVoice) localStorage.setItem(AZURE_KEY, azureVoice);
    } catch {
      /* */
    }
  }, [azureVoice]);
  useEffect(() => {
    try {
      if (hfVoice) localStorage.setItem(HF_KEY, hfVoice);
    } catch {
      /* */
    }
  }, [hfVoice]);
  useEffect(() => {
    try {
      localStorage.setItem(PLAYHT_KEY, playHtVoice);
    } catch {
      /* */
    }
  }, [playHtVoice]);
  useEffect(() => {
    try {
      localStorage.setItem(OPENTTS_KEY, openTtsVoice);
    } catch {
      /* */
    }
  }, [openTtsVoice]);
